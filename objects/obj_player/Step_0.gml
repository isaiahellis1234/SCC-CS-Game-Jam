// If room == Room_Cutscene_1 (Cutscene Logic)
...
// If room != Room_Cutscene_1 (Regular game logic)
if room != Room_Cutscene_1 {
    ...
    // Grappling Physics
    if (active) {
        // Check if grappling block still exists, if not, stop grappling
        if (!position_meeting(mx, my, obj_grappling_block)) {
            active = false;
            jump_count = 1;
        }
